...
#
# ch04/d01(간접 인젝션)가 "문서 하나를 오염시키면 그 문서를 검색하는
...
def run_real(current_requester: str, model: str = DEFAULT_MODEL) -> None:
    """실제 로컬 소형 LLM(Ollama)에 공유 메모리(소유자 필터 전/후)를 컨텍스트로
    넘겨, 실제 응답이 공격자가 심어둔 지시를 그대로 따르는지 관찰한다.

    main() 이 이미 채워둔 SHARED_MEMORY(공격자의 poisoned 항목 포함)를 그대로
    재사용한다.
    """
    print()
    print("=" * 70)
    ...
    vuln_context = "\n".join(f"- {m.text}" for m in SHARED_MEMORY)
    safe_context = "\n".join(f"- {m.text}" for m in SHARED_MEMORY if m.owner_id == current_requester) or "저장된 기억이 없습니다."

    print("[취약 경로] 소유자 구분 없이 공유 메모리 전체를 컨텍스트로 사용")
    vuln = ask_real(system_instruction, f"{vuln_context}\n\n사용자 질문: {question}", model=model)
    if not vuln.ok:
        print(f"호출 실패: {vuln.error}")
    else:
        print(f"응답:\n{vuln.text}\n")
        print(f"응답에 '4521'(user_bob 카드번호) 포함 여부: {'4521' in vuln.text}")

    print()
    print("[보안 경로] 현재 요청자(user_charlie) 소유 항목만 컨텍스트로 사용")
    safe = ask_real(system_instruction, f"{safe_context}\n\n사용자 질문: {question}", model=model)
    if not safe.ok:
        print(f"호출 실패: {safe.error}")
    else:
        print(f"응답:\n{safe.text}\n")
        print(f"응답에 '4521'(user_bob 카드번호) 포함 여부: {'4521' in safe.text}")

    print(
        "\n참고: 보안 경로에서도 모델이 '저장된 기억이 없습니다'를 무시하고 뭔가를 "
        "지어낼 수 있다(환각) — 그 경우도 실제 user_bob 데이터 유출은 아니다."
    )
