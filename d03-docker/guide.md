...
`ch04/d01-docker`(프롬프트 인젝션)가 "공격자의 지시를 모델이 따르는가"를, `ch04/d02-docker`
...
| `Dockerfile` | 의존성 없이 컨테이너에서 실행하기 위한 이미지 정의 (`d01-docker`/`d02-docker` 패턴) |  |
...
### Docker
```
docker build -t llm-data-leakage-demo .
