...
외부 API 키 없이 재현 가능한 jailbreak(탈옥) 실습 3건. `ch04/d01-docker`(프롬프트
...
| `Dockerfile` | 의존성 없이 컨테이너에서 실행하기 위한 이미지 정의 (`ch04/d01-docker` 패턴 참고) |
...
```
sudo docker build -t jailbreak-defense-demo .
```
세 예제 순차 실행
```
...
예제 1만 실행                     
```
sudo docker run --rm jailbreak-defense-demo python persona_jailbreak.py
```
...
- 세 예제 모두 mock 기반 결정론적 시뮬레이션이다. 실제 LLM 은 모델·프롬프트 버전마다
  ...
  (`ch04/local-llm` 의 방식처럼 실제 소형 LLM으로 재현해볼 수 있다).
