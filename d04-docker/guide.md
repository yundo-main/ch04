...
`ch04/d03-docker`(데이터 유출)가 "ACL이 아예 없을 때 무슨 일이 생기는가"를 다뤘다면,
...
| `Dockerfile` | 의존성 없이 컨테이너에서 실행하기 위한 이미지 정의 (`d01~d03-docker` 패턴) |  |
...
- **배경**: 강의안 경고 박스 — "Post-filter만 쓰면 거절 전 점수나 타이밍으로 존재 여부가
  샐 수 있다." `d03-docker`의 예제 2(교차 테넌트)와 다른 지점: 거기서는 문서 **내용**이
